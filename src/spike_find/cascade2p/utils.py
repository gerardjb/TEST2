...
def calibrated_ground_truth_artificial_noise(ground_truth_folder,noise_level,sampling_rate,replicas,omission_list=[], verbose=3):
    ...
    for file_index,neuron_file in enumerate(fileList):  
      ...
      print(f"Loading data for {neuron_file}")
      ...
      for i,trial in enumerate(dataset_neuron_all):
          ...
          try:
            ...
            #print("keys_unfolded = ",keys_unfolded)
            ...
            #print("fluo_times[-1] = ",fluo_times[-1])
            #print("traces_mean[-1]",traces_mean[-1])
            ...
            #print("noise_level, base_noise, frame_rate, sampling_rate = ",noise_level, base_noise, frame_rate, sampling_rate)
            ...
            if nb_subROIs >= 1:
              ...
              for iii in range(int(nb_subROIs)):
                ...
                #print("events/ephys_sampling_rate = ",events/ephys_sampling_rate)
                counter += 1

            
          except Exception as e:
            if verbose>2:
              print("While attempting to load noise-matched samples got error:")
              print(e)
            pass
      ...
      try:
        #print("sub_traces = ",sub_traces)
        events_all[file_index] = events_all[file_index][0:sub_traces.shape[1]]
      except Exception as e:
        if verbose>2:
          print("While loading events for this file, got error:")
          print(e)
        pass
...
def preprocess_groundtruth_artificial_noise_balanced(ground_truth_folders,before_frac,windowsize,after_frac,noise_level,sampling_rate,smoothing,omission_list=[],permute=1,maximum_traces=5000000,verbose=3,replicas=1,causal_kernel=0):
    ...
    for dataset_index,training_dataset in enumerate(ground_truth_folders):
      ...
      try:
        ...
        #print("sub_traces_allX.shape = ",sub_traces_allX.shape)
        datapoint_counter = 0
        for k in range(len(sub_traces_allX)):
          try:
            ...
            if verbose > 2:
              ...
              print(e)
        #print("datapoint_counter = ",datapoint_counter)
        ...
        if verbose>2:
           print("<<<<<<<<<<<<<<<<<<<<<<<<<Caught exception in 'Preprocessing dataset...' block")
           print(e)
